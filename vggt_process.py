...
def extract_frames_from_video(video_path, output_dir, fps_extraction=1.0):
    """
    Extract frames from a video file and save them as images.
    Based on the frame extraction logic from demo_gradio.py.
    
    Args:
        video_path: Path to the input video file
        output_dir: Directory to save extracted frames
        fps_extraction: Frames per second to extract (default: 1.0 = 1 frame/sec)
    ...
    frame_interval = int(fps / fps_extraction)  # Extract at specified rate
    
    print(f"Video FPS: {fps}, extracting every {frame_interval} frames ({fps_extraction} frames/sec)")
    ...
    while True:
        ...
        if count % frame_interval == 0:
            image_path = os.path.join(output_dir, f"frame_{video_frame_num:06d}.png")
            cv2.imwrite(image_path, frame)
            image_paths.append(image_path)
            video_frame_num += 1
    
    vs.release()
    print(f"Extracted {len(image_paths)} frames to {output_dir}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert images or video to COLMAP format using VGGT",
        epilog="""
Examples:
  # Process existing images:
  python vggt_process.py --image_dir path/to/images --output_dir colmap_output
  
  # Process video (auto-create image directory):
  python vggt_process.py --video_file video.mp4 --output_dir colmap_output
  
  # Process video with custom frame extraction directory:
  python vggt_process.py --video_file video.mp4 --image_dir my_frames --fps 2.0
        """,
    ...
    parser.add_argument("--image_dir", type=str, 
                        help="Directory containing input images (or where to extract video frames)")
    parser.add_argument("--video_file", type=str,
                        help="Path to input video file (will extract frames to --image_dir)")
    ...
    if not args.video_file and not args.image_dir:
        parser.error("Either --video_file or --image_dir must be specified")
    
    # Handle video input
    if args.video_file:
        if not os.path.exists(args.video_file):
            raise ValueError(f"Video file not found: {args.video_file}")
        
        # Create image_dir if not specified
        if not args.image_dir:
            # Use video filename (without extension) as default image_dir
            video_basename = os.path.splitext(os.path.basename(args.video_file))[0]
            args.image_dir = f"extracted_frames_{video_basename}"
            print(f"No --image_dir specified, using: {args.image_dir}")
        
        # Extract frames from video
        extract_frames_from_video(args.video_file, args.image_dir, args.fps)
    
    # Validate image_dir exists and has images
